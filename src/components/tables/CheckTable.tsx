...
export default function App({ usersData }: { usersData: UsersHome[] }) {
  ...
  const sortedItems = React.useMemo(() => {
    return [...items].sort((a: UsersHome, b: UsersHome) => {
  ...
  }, [usersData.length, page, pages, hasSearchFilter]);
      ...
      <TableBody emptyContent={"No users found"} items={sortedItems}>
        {(usersData) => (
          <TableRow key={usersData.id}>
            {(columnKey) => <TableCell>{renderCell(usersData, columnKey)}</TableCell>}
          </TableRow>
        )}
      </TableBody>
    </Table>
